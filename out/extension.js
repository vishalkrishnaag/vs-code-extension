...
function validateImports(document, tokens, diagnostics) {
...
function validateImportPath(document, documentDir, pathToken, diagnostics) {
...
function isMapKey(tokens, index) {
    let depth = 0;
    for (let i = index - 1; i >= 0; i--) {
        const kind = tokens[i].kind;
        if (kind === "rbrace" || kind === "rbracket" || kind === "rparen")
            depth++;
...
function isNamedArgument(tokens, index) {
    let depth = 0;
    for (let i = index - 1; i >= 0; i--) {
        const kind = tokens[i].kind;
        if (kind === "rbrace" || kind === "rbracket" || kind === "rparen")
...
function validateClauseHeadFields(document, tokens, diagnostics) {
    ...
    for (let i = 0; i < tokens.length - 1; i++) {
        ...
        for (const name of globalBindings)
            declared.add(name);
        for (const name of importedModules)
            declared.add(name);
        let depth = 0;
        let argStart = i + 2;
        let argValueStart;
...
function containsMemberAccess(tokens, start, end) {
...
function statementEndIndex(tokens, start) {
...
function collectVariableNames(tokens, start, end) {
    ...
    for (let i = start; i < end; i++) {
        ...
        if (token.text === "nil")
            continue;
        if (["else", "extend", "where", "return", "lambda", "then"].includes(token.text))
            continue;
...
function collectHeadDeclaredVars(tokens, start, end, methodStyle) {
    ...
    for (let i = start; i <= end; i++) {
        ...
        if (i === end || (token.kind === "comma" && depth === 0)) {
            ...
            if (valueStart !== undefined) {
                ...
                if (methodStyle && value?.kind === "ident") {
                    ...
                    for (const name of collectVariableNames(tokens, valueStart, i))
                        declared.add(name);
                }
            }
            else if (!methodStyle) {
                for (const name of collectVariableNames(tokens, argStart, i))
                    declared.add(name);
            }
            argStart = i + 1;
            nameStart = undefined;
        ...
        if (depth !== 0)
            continue;
        if (i === argStart && token.kind === "ident" && tokens[i + 1]?.kind === "colon")
            nameStart = i;
        if (i === argStart + 1 && token.kind === "colon")
            valueStart = i + 1;
    }
    return declared;
}
function validateBodyDeclaredVars(document, tokens, start, end, declared, diagnostics) {
    let segmentStart = start;
    let depth = 0;
    const validateSegment = (from, to) => {
        while (from < to && tokens[from].kind === "comma")
            from++;
        while (from < to && tokens[to - 1]?.kind === "comma")
            to--;
        if (from >= to)
            return;
...
function validateStatements(document, tokens, diagnostics) {
    const parenStack = [];
    let statementStart;
    let previous;
    for (let i = 0; i < tokens.length; i++) {
        const token = tokens[i];
        statementStart ?? (statementStart = token);
        if (token.kind === "lparen" || token.kind === "lbrace" || token.kind === "lbracket") {
            ...
            if (parenStack.length === 0) {
                ...
                const matches = (open?.kind === "lparen" && token.kind === "rparen") ||
...
function validateCalls(document, tokens, diagnostics) {
...
function resolveInterpreterPath(documentUri) {
    ...
    return resolveReleaseExecutable(documentUri, config.get("interpreterPath", ""), "felidae_vm");
}
function resolveCompilerPath(documentUri) {
    const config = vscode.workspace.getConfiguration("felidae");
    return resolveReleaseExecutable(documentUri, config.get("compilerPath", ""), "felidae_compiler");
}
...
function resolveDebugInterpreterPath(documentUri) {
    ...
    const config = vscode.workspace.getConfiguration("felidae");
    return resolveReleaseExecutable(documentUri, config.get("debugInterpreterPath", ""), "felidae_debugger");
}
...
async function confirmRuntimeCheck(document, actionLabel) {
    ...
    if (!hasErrors)
        return true;
    const choice = await vscode.window.showWarningMessage(`felidae_debug --check-json reported errors. ${actionLabel} anyway?`, `${actionLabel} Anyway`, "Cancel");
...
class FelidaeDebugAdapter {
    ...
    launch(request) {
        ...
        if (compilation.stdout)
            this.sendOutput(compilation.stdout, "stdout");
        const launchArgs = [program];
        if (stopOnEntry)
            launchArgs.push("--stop-on-entry");
        if (query)
            launchArgs.push("--query", query);
    ...
    resolveImportFiles(fromFile, importPath) {
    ...
    stepSimulated(command) {
        this.stopped = true;
        if (command === "stepIn" && this.stepIntoCall())
            return;
        if (command === "stepOut" && this.stepOutOfCall())
            return;
        this.stepToNextLine();
    }
    stepToNextLine() {
    ...
    findMethodCallOnLine(line) {
        ...
        for (const match of calls) {
            ...
            if (ignored.has(base) || ignored.has(name))
                continue;
            if (this.methodDefinitions.has(name))
                return name;
        }
        return undefined;
    }
    ...
    collectLineVariables(line, variables) {
        const withoutComment = line.split("#", 1)[0];
...
class FelidaeDebugConfigurationProvider {
    resolveDebugConfiguration(folder, config) {
        ...
        config.interpreterPath ?? (config.interpreterPath = resolveDebugInterpreterPath(anchor));
        config.compilerPath ?? (config.compilerPath = resolveCompilerPath(anchor));
