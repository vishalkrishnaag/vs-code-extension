...
function leadingWidth(rawLine: string): number {
  let width = 0;
...
export function formatFelidaeLines(rawLines: readonly string[]): string[] {
  ...
  for (const rawLine of rawLines) {
    ...
    if (isBareEnd) {
      ...
      let idx = 0;
      while (idx < masked.length && (masked[idx] === " " || masked[idx] === "\t")) idx++;
...
export function formatFelidaeSource(text: string): string {
